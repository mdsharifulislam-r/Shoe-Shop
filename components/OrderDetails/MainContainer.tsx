...
async function  Item({id}:{id:number}){
  ...
  const item:productCard = data?.success?data?.data:{}
  return (
    <div className="mt-4 md:mt-6 flex  flex-col md:flex-row justify-start items-start  md:space-x-6 xl:space-x-8 w-full ">
    <div className=" w-full md:w-40 bg-slate-200 md:h-28 my-2 h-60 flex justify-center place-items-center">
      <Image src={item?.image||shoe} className='w-[70%]' alt='' width={300} height={300}/>
    ...
    <div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full  pb-8 space-y-4 md:space-y-0">
        <div className="w-full flex flex-col justify-start items-start space-y-8">
            ...
            <div className="flex justify-start items-start flex-col space-y-2">
                ...
                <p className="text-sm leading-none text-gray-800">
                    <span className="text-gray-300">Price: </span> {item?.price}
                </p>
        ...
        <div className="flex justify-between space-x-8 items-start w-full">
            ...
            <p className="text-base xl:text-lg font-semibold leading-6 text-gray-800 pl-14">${item?.price}</p>
        </div>
    </div>
</div>
  )
...
export default async function MainContainer({id}:{id:string}) {
  const res = await fetch(`${process.env.BASE_URL}/orders/${id}`)
  const data = await res.json()
  const item:orderType = data?.success?data?.data:{}
  const product_ids = item?.products?.split(",").map(Number)||[]
  ...
  return (
    ...
    <div className="flex justify-start item-start space-y-2 flex-col ">
        ...
        <p className="text-base font-medium leading-6 text-gray-600">{item?.order_date}</p>
    </div>
            ...
            <div className="flex justify-center md:flex-row flex-col items-stretch w-full space-y-4 md:space-y-0 md:space-x-6 xl:space-x-8">
                <div className="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 space-y-6   ">
                    ...
                    <div className="flex justify-between items-center w-full">
                        ...
                        <p className="text-base font-semibold leading-4 text-gray-600">${item?.net_price}</p>
                    </div>
                </div>
                <div className="flex flex-col justify-center px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 space-y-6   ">
                    <h3 className="text-xl font-semibold leading-5 text-gray-800">{item.status}</h3>
                ...
                <div className="flex justify-between xl:h-full  items-stretch w-full flex-col mt-6 md:mt-0">
                    <div className="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row  items-center md:items-start ">
                        <div className="flex justify-center md:justify-start  items-center md:items-start flex-col space-y-4 xl:mt-8">
                            ...
                            <p className="w-48 lg:w-full xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">{item?.address}</p>
                        </div>
