...
const isAdShow: boolean = getStorLocal("ad")||false;
...
const cartSlice = createSlice({
  ...
  reducers: {
    addCartItem: (state, action) => {
      if (
        state.cart.some((item) => item.primary_id == action.payload?.primary_id)
      ) {
        const cart_id = action.payload.primary_id;
        const index = state.cart.findIndex(
          (item) => item.primary_id == cart_id
        );
        state.cart[index] = {
          ...state.cart[index],
